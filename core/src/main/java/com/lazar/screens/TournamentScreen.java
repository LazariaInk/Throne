...
public class TournamentScreen implements Screen {
    ...
    private void drawMazeOnly() {
        shapeRenderer.begin(ShapeRenderer.ShapeType.Filled);
        float haloAlpha = mazeWon ? 0.18f + 0.05f * MathUtils.sin(infoPulseTimer * 3f) : 0.08f;
    ...
    private void drawRingWithGap(float cx, float cy, float radius, float thickness, float gapCenterDeg, float gapSizeDeg, Color color) {
